...
function BuffLib:CHAT_MSG_ADDON(prefix, message, channel, sender)
	--log(prefix .. "  " .. message .. "  " .. channel .. "  " .. sender)
	if prefix == "BuffLib" and sender ~= UnitName("player") then
		...
		if EBFrame ~= nil then
			...
			if not self.guids[guid] then self.guids[guid] = {} end
		...
		if XPerl_Target and XPerl_Target:IsVisible() and guid == UnitGUID("target") then
			...
			XPerl_Target_DebuffUpdate(XPerl_Target)
		end
		if XPerl_Focus and XPerl_Focus:IsVisible() and guid == UnitGUID("focus") then
			XPerl_Targets_BuffUpdate(XPerl_Focus)
			XPerl_Target_DebuffUpdate(XPerl_Focus)
		end
		if XPerl_Player_Pet and XPerl_Player_Pet:IsVisible() and UnitGUID("pet") == guid then
			XPerl_Player_Pet_Buff_UpdateAll(XPerl_Player_Pet)
		end
		if XPerl_TargetTarget and UnitGUID("targettarget") == guid then
			XPerl_TargetTarget_Buff_UpdateAll(XPerl_TargetTarget)
		end	
		for i=1,GetNumPartyMembers() do
			if guid == UnitGUID("party"..i) and getglobal("XPerl_party"..i) then
