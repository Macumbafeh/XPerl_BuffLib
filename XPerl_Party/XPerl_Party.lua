...
function XPerl_Party_Events_OnLoad(self)
	local events = {"PLAYER_ENTERING_WORLD", "PARTY_MEMBER_ENABLE", "PARTY_MEMBER_DISABLE", "RAID_ROSTER_UPDATED", "PARTY_MEMBERS_CHANGED",
			...
			"UNIT_TARGET", "UNIT_RAGE", "UNIT_MAXRAGE", "UNIT_ENERGY", "UNIT_MAXENERGY", "UNIT_MANA", "UNIT_MAXMANA",
			"UNIT_HEALTH", "UNIT_MAXHEALTH", "UNIT_LEVEL", "UNIT_DISPLAYPOWER", "UNIT_NAME_UPDATE", "PLAYER_FLAGS_CHANGED",
			"RAID_TARGET_UPDATE", "READY_CHECK", "READY_CHECK_CONFIRM", "READY_CHECK_FINISHED", "PLAYER_LOGIN"}
...
function XPerl_Party_UpdateHealth(self)
	local Partyhealth, Partyhealthmax = XPerl_UnitHealth(self.partyid), UnitHealthMax(self.partyid)
	local reason

	if (self.feigning and not UnitIsFeignDeath(self.partyid)) then
		self.feigning = nil
	end
	...
	if (not UnitIsConnected(self.partyid)) then
		...
		elseif (UnitIsDead(self.partyid)) then
...
function XPerl_Party_Buff_UpdateAll(self)
...
local function XPerl_Party_UpdatePVP(self)
	if (pconf.pvpIcon and UnitIsPVP(self.partyid)) then
		self.nameFrame.pvpIcon:SetTexture("Interface\\TargetingFrame\\UI-PVP-"..(UnitFactionGroup(self.partyid) or "FFA"))
...
local function CheckRaid()
	if (InCombatLockdown()) then
		...
		if (pconf.inRaid or (pconf.smallRaid and singleGroup) or GetNumRaidMembers() == 0) then
